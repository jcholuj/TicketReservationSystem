...
class SearchInputs extends React.Component {
	...
	switchStations = () => {
		...
		if (!Object.values(vals).every((x) => x === undefined || x === "")) {
			const switched = {
	...
	disabledDateTime() {
		...
		return {
			disabledMinutes: () =>
				range(0, 59).filter((n) => allowedMin.indexOf(n) === -1),
		};
	}
	...
	render() {
		...
		return (
			<React.Fragment>
				<Form.Item name='connect_date' initialValue={this.getTimeAdjustNow()}>
					<DatePicker
					...
					<Col span={checkOption1 ? 10 : 15}>
						<Form.Item name='connect_orig' noStyle>
							<AutoComplete
								...
								placeholder={checkOption1 ? "Origin" : "Station"}
					...
					{checkOption1 && (
						<Col span={4} className='stations-btn-col'>
							<Button
								...
								icon={
									<SwapOutlined
										rotate={this.state.switch_triggered ? 180 : 0}
									/>
								}
								size='large'
